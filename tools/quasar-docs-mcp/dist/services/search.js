...
function calculateScore(page, queryTerms) {
    let score = 0;
...
export async function searchContent(query, paths, limit = 5) {
    const queryTerms = normalizeQuery(query);
    if (queryTerms.length === 0) {
        return [];
    }
...
export function filterBySection(index, section) {
    ...
    return index.pages.filter((page) => page.section.toLowerCase() === sectionLower ||
