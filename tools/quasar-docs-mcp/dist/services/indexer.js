...
function extractKeywordsFromMarkdown(content, path) {
    const keywords = [];
...
export async function buildIndex() {
    ...
    for (const filePath of files) {
        ...
        const title = content ? extractTitleFromMarkdown(content) : filePath.split("/").pop()?.replace(".md", "") || "";
...
export async function buildLightweightIndex() {
    const files = await fetchAllMarkdownFiles();
    const sections = [];
    const pages = [];
    const seenSections = new Set();
