...
export const searchDocsSchema = z.object({
    ...
    section: z
        .string()
        .optional()
        .describe("Optional: limit search to a specific section (e.g., 'vue-components', 'style', 'quasar-plugins')"),
    limit: z
        .number()
        .min(1)
        .max(20)
...
export async function searchDocs(input) {
    ...
    if (input.includeContent && results.length < input.limit) {
        const pathsToSearch = pagesToSearch
            ...
            .map((p) => p.path);
        const contentResults = await searchContent(input.query, pathsToSearch, input.limit - results.length);
    ...
    if (results.length === 0) {
        return `No results found for '${input.query}'.${input.section ? ` Try removing the section filter or searching in a different section.` : ""}`;
    }
    // Format results
    const formatted = results
        .map((r, i) => `${i + 1}. **${r.title}**\n   Section: ${r.section}\n   Path: ${r.path}\n   URL: ${r.url}${r.snippet && r.snippet !== `Section: ${r.section}` ? `\n   Snippet: ${r.snippet}` : ""}`)
        .join("\n\n");
