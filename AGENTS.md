...
- The production PWA is deployed on Cloudflare Workers static assets at `https://shephard.app`, configured by `wrangler.jsonc` and `public/_headers`. Keep the OAuth consent callback at `https://shephard.app/oauth/consent` and use that origin for MCP browser-origin configuration.
